...
class BL_UI_OT_draw_operator(Operator):
    ...
    def register_handlers(self, args, context):
        if self.draw_area == 'VIEW_3D':
            self.draw_handle = bpy.types.SpaceView3D.draw_handler_add(self.draw_callback_px, args, "WINDOW",
                                                                      "POST_PIXEL")
        else:
            self.draw_handle = bpy.types.SpaceNodeEditor.draw_handler_add(self.draw_callback_px, args, "WINDOW",
                                                                          "POST_PIXEL")
        self.draw_event = context.window_manager.event_timer_add(0.005, window=context.window)
